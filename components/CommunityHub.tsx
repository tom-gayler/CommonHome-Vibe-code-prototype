...
export const CommunityHub: React.FC<CommunityHubProps> = ({ leaseholders, onUpdate }) => {
  ...
  const filteredLeaseholders = leaseholders.filter(l => {
    ...
    if (filter === 'signed') return l.status === 'signed';
    if (filter === 'missing_contact') return l.contact_status === 'missing';
    if (filter === 'non_resident') return !l.is_resident;
    return true;
  });
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex bg-slate-100 p-1.5 rounded-2xl shadow-inner border border-slate-50 w-full md:w-fit">
        {(['all', 'signed', 'non_resident', 'missing_contact'] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredLeaseholders.map((leaseholder) => (
          <div key={leaseholder.id} className="bg-white p-7 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-[#4c6fa1]/5 hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
            {leaseholder.contact_status === 'missing' && (
            ...
            <div className="flex justify-between items-start mb-6 mt-2">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-white shadow-lg ${
                  leaseholder.status === 'signed' ? 'bg-brand-gradient' :
                  leaseholder.status === 'opposed' ? 'bg-slate-800' :
                  'bg-slate-200'
                }`}>
                  {leaseholder.display_name.charAt(0)}
                </div>
                <div>
                  <h4 className="font-extrabold text-[#333333] text-lg leading-tight">{leaseholder.display_name}</h4>
                  <div className="flex items-center gap-2 mt-1">
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">{leaseholder.unit_label}</p>
                    {!leaseholder.is_resident && (
            ...
            <div className="flex gap-3 mt-4 pt-6 border-t border-slate-50">
              {leaseholder.contact_status === 'missing' ? (
                <button
                  disabled={updating === leaseholder.id}
                  onClick={() => handleUpdate(leaseholder.id, { contact_status: 'available' })}
                  className="flex-1 py-3 text-xs font-black text-white bg-amber-500 rounded-xl hover:bg-amber-600 transition-colors uppercase tracking-wider shadow-md shadow-amber-200 disabled:opacity-50"
                >
                  {updating === leaseholder.id ? '...' : 'Add Details'}
                </button>
              ) : (
              ...
              {leaseholder.status === 'pending' && leaseholder.contact_status !== 'missing' && (
                <button
                  disabled={updating === leaseholder.id}
                  onClick={() => handleUpdate(leaseholder.id, { status: 'signed' })}
                  className="flex-1 py-3 text-xs font-black text-[#d94e6d] bg-[#d94e6d]/5 rounded-xl hover:bg-[#d94e6d]/10 transition-colors uppercase tracking-wider disabled:opacity-50"
                >
                  {updating === leaseholder.id ? '...' : 'Remind'}
                </button>
              )}
            </div>
          </div>
      ...
      <div className="bg-[#fdf8f9] rounded-[2.5rem] p-10 flex flex-col md:flex-row items-center gap-10 border border-[#d94e6d]/10 relative overflow-hidden">
        ...
        <div className="hidden lg:block w-48 text-9xl opacity-20">📣</div>
      </div>
    </div>
  );
};
